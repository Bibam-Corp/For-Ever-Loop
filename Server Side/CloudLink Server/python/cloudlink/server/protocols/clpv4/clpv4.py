    ...
    def __init__(self, server):
        ...
        * cf-connecting-ip
        ...
        async def protocol_identified(client):
            ...
            server.rooms_manager.subscribe(client, self.default_room)
        ...
        async def on_setid(client, message):
            ...
            # Leave default room
            server.rooms_manager.unsubscribe(client, self.default_room)
            ...
            clients = await server.rooms_manager.get_all_in_rooms(self.default_room, cl4_protocol)
            clients = server.copy(clients)
            clients.remove(client)
            server.send_packet(clients, {
                "cmd": "ulist",
            ...
            server.send_packet(client, {
                "cmd": "ulist",
                "mode": "set",
                "val": server.rooms_manager.generate_userlist(self.default_room, cl4_protocol),
                "rooms": self.default_room
            })
        ...
        async def on_link(client, message):
            ...
            if self.default_room not in message["val"]:
                server.rooms_manager.unsubscribe(client, self.default_room)
